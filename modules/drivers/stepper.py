...
class StepperMotor:
  ...
  def __init__(self, dir_pin, step_pin, speed=60):
    self.delay_bw_steps = calc_delay_for_speed(speed)
    ...
    self.delay_per_step = get_step_delay_from_speed(speed)
    
  def get_step_delay_from_speed(speed):
    """ Converts the speed (in rpm) to the delay between each step """
    
    rotations_per_second = speed / 60
    steps_per_second = rotations_per_second * steps_per_rotation
    time_per_step = float(1.0)/steps_per_second
    return time_per_step - min_delay_per_step
  ...
  def rotate_clockwise(self, speed, num_rotations):
    self.rotate_at_speed(speed, True, num_rotations)

  def rotate_anticlockwise(self, speed, num_rotations):
